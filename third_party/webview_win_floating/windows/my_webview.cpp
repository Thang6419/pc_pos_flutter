...
HRESULT MyWebViewImpl::addScriptChannelByName(LPCWSTR channelName)
{
    ...
    if (wcscmp(channelName, L"pcPosWindowsBridge") == 0) {
        LPCWSTR bridgeScript = LR"JS(
            (function() {
        ...
        HRESULT bridgeHr = m_pWebview->AddScriptToExecuteOnDocumentCreated(
            bridgeScript,
            Callback<ICoreWebView2AddScriptToExecuteOnDocumentCreatedCompletedHandler>(
                [](HRESULT error, PCWSTR id) -> HRESULT {
                    return error;
                }).Get());
        if (FAILED(bridgeHr)) return E_FAIL;
    }
