    ...
    InitWebViewRuntime(pwUserDataFolder, [=](HRESULT hr) -> void {
        ...
        auto onControllerCreated = Callback<ICoreWebView2CreateCoreWebView2ControllerCompletedHandler>(
            [=](HRESULT hr, ICoreWebView2Controller* controller) -> HRESULT {
                ...
                // Keep normal page scrolling, but disable WebView2's native
                // multi-touch zoom and swipe-navigation gestures. These
                // gestures can make a touch-screen POS page appear to move or
                // rotate independently from the web application's own input.
                if (auto settings5 = m_pSettings.try_query<ICoreWebView2Settings5>()) {
                    settings5->put_IsPinchZoomEnabled(FALSE);
                }
                if (auto settings6 = m_pSettings.try_query<ICoreWebView2Settings6>()) {
